...
def main():
     ...
     ndo = ndo.asfreq('d')
     ndo = ndo.to_period()
     ndo15 = rhistinterp(ndo,'h',lowbound=-2000.)
     ...
     x_res, coefs, pred_df = fit_mrzecest_gee(config, elev=elev, ndo=ndo15, ec_obs=obs_ec15)

     log10beta = x_res[0] # x[0] from ec_boundary_fit_gee.py printout
     npow = x_res[1] # x[1] from ec_boundary_fit_gee.py printout
     area_coef = x_res[2]*3600*1000000. # x[2] from ec_boundary_fit_gee.py printout
     energy_coef = x_res[3]*1000. # x[3] from ec_boundary_fit_gee.py printout
     ...
     filt_coefs = [ak* 1e-3 for ak in coefs[coefs.index.str.startswith('z')].values] # z{n} from output coefs
     ...
     mrzecest = ec_est(ndo15, elev, start, end,
