...
def find_ecest_params(elev, ndo, ec_obs, config)
  
    """ Fit the model given time series of outflow, elev and observed ec and config file of paramters
    ...
    # set up filter
    filter_dt = config('filter_setup']['filter_length']
    filter_k0 = int(config('filter_setup']['ko'])
    dstep = int(filter_dt/pd.Timedelta(ndo.index.freq)) # number of rows for each Dt
...
def fit_mrzec_model(config, df_ndo=None, df_mtz_elev=None): 
